...
z=w.innerHTML,/<br>/i.test(z)&&(
// Firefox adds <br>'s and so we remove the <br>
z=z.replace(/<br>/i,"")),w.innerHTML=z))}catch(a){}
//console.log('************** selectedElement:', selectedElement);
var B=angular.element(w),C=w.tagName.toLowerCase();if("insertorderedlist"===m.toLowerCase()||"insertunorderedlist"===m.toLowerCase()){var D=c("insertorderedlist"===m.toLowerCase()?"ol":"ul");if(C===D)
...
t[q].parentNode&&t[q].parentNode.removeChild(t[q])}else/* istanbul ignore next: not tested since identical to blockquote */
if("pre"===v&&b.getStateShiftKey()){for(
...
/* check if selection is a BR element at the beginning of a container. If so, get
		...
		* element.
		*/
return a.tagName&&a.tagName.match(/^br$/i)&&0===b&&!a.previousSibling?{element:a.parentNode,offset:0}:{element:a,offset:b}},h={getSelection:function(){var b=a.getSelection().getRangeAt(0),c=b.commonAncestorContainer,d={start:g(b.startContainer,b.startOffset),end:g(b.endContainer,b.endOffset),collapsed:b.collapsed};
...
l=[],j=0;j<s.length;j++){var u=s[j];"p"===u.nodeName.toLowerCase()&&""===u.innerHTML.trim()||(3!==u.nodeType||u.nodeValue!=="\ufeff"[0]||""!==u.nodeValue.trim()?3===u.nodeType&&""===u.nodeValue.trim()||(t=t&&!i.test(u.nodeName),l.push(u)):
// no change to isInline
l.push(u))}for(var v=0;v<l.length;v++)n=r.appendChild(l[v]);!t&&q.collapsed&&/^(|<br(|\/)>)$/i.test(q.startContainer.innerHTML)&&q.selectNode(q.startContainer)}else t=!0,
...
if(!(""===A||a.keyCode!==Q||a.ctrlKey||a.metaKey||a.altKey||a.shiftKey)){
// shifted to nodeName here from tagName since it is more widely supported see: http://stackoverflow.com/questions/4878484/difference-between-tagname-and-nodename
for(
